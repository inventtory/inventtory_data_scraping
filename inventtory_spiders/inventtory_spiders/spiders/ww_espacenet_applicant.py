'''
    File name: ww_espacenet_applicant.py
...
class EspacenetSpider(CrawlSpider):
    name = "espacenet_applicant"
    def start_requests(self):
        #you can enter multiple applicants - however, they must all feature in a patent document
        patent_applicants = ['WHITE ALLAN G','PREVAR LIMITED']
        applicants = ''
        for applicant in patent_applicants:
            applicants = applicants + '+' + str(applicant)
        url = 'https://worldwide.espacenet.com/data/searchResults?submitted=true&locale=en_EP&DB=EPODOC&ST=advanced&TI=&AB=&PN=&AP=&PR=&PD=&PA=' + str(applicants) + '&IN=&CPC=&IC=&Submit=Search'
