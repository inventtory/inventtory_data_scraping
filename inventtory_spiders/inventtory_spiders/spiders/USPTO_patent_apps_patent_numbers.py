'''
    File name: USPTO_patent_apps_patent_numbers.py
...
class UsptoSpider(CrawlSpider):
    name = "uspto_patent_app_numbers"
    
    def start_requests(self):
        patent_app_nums = ['20160345466','20160345385','20160345328','20160345322','20160345303','20160345263']
        patent_apps = ''
        #example query string: 'DN/20160345466 OR DN/20160345385 OR DN/20160345328 OR DN/20160345322 OR DN/20160345303 OR DN/20160345263'
        for patent_app in patent_app_nums:
            patent_apps = patent_apps + 'DN/' + str(patent_app) + ' OR '
        patent_apps = patent_apps[:-4]  
        url = 'http://appft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&u=%2Fnetahtml%2FPTO%2Fsearch-adv.html&r=0&p=1&f=S&l=50&Query=' + str(patent_apps) + '&d=PG01'
    ...
    def parse_next_page(self, response):
        ...
        links = ['http://appft.uspto.gov' + str(e) for e in links]
        for patent_link in links:
            yield Request(patent_link, callback=self.parse_patent, dont_filter=True)


    def parse_patent(self, response):

        ###TEST FOR PAGE ERROR###
        if response.xpath('//h1[contains(text(), "Error!")]'):
        ...
        #1 patent_application_number
        patent_application_number = response.xpath('//table[2]//tr//td[2]//text()').re('(\d{11}).*')[0]
        item['patent_application_number'] = patent_application_number
        ...
        if response.xpath('//font[@size="+1"]/text()') != []:
            patent_name = response.xpath('//font[@size="+1"]//text()').extract()
            item['patent_name'] = ''.join(patent_name)
        else:
            item['patent_name'] = ''
        ...
        #4 patent_type
        patent_kind_code = response.xpath('//table[2]//tr[2]//td/b/text()').extract()[1]
        patent_kind_code = patent_kind_code.replace('\n','')
        #patent_kind_code = [re.match('([A-Z]{1}[0-9]{1}|A1|A2).*\n*.*',e).group(1) for e in patent_kind_code if re.match('([A-Z]{1}[0-9]{1}|A1|A2).*\n*.*',e)][0]
        item['patent_kind_code'] = patent_kind_code

        #key_identifier
        key_identifier = str(patent_country_code) + str(patent_application_number) + str(patent_kind_code)
        ...
        #6 inventors
        if response.xpath('//table[3]//tr[1]//text()').extract() != []:
            inventors = response.xpath('//table[3]//tr[1]//text()').extract()
            clean = [e for e in inventors if '\n' not in e]
            clean = [e for e in clean if 'Inventors' not in e]
        ...
        #17 current_US_class
        if response.xpath('//tr').re('Current\s*U.S.\s*Class:.*(?:\n.*)+?<b>(\d+\/\d+.*)+<\/td>.*') != []:
            item['current_US_class'] = response.xpath('//tr').re('Current\s*U.S.\s*Class:.*(?:\n.*)+?<b>(\d+\/\d+.*)+<\/td>.*')[0]
        else:
            item['current_US_class'] = ''

        #17b current_cpc_class - test for this
        if response.xpath('//tr').re('Current\s*CPC\s*Class:.*(?:\n.*)+?<td valign="TOP".*?>(.*?)<\/td>.*') != []:
            item['current_CPC_class'] = response.xpath('//tr').re('Current\s*CPC\s*Class:.*(?:\n.*)+?<td valign="TOP".*?>(.*?)<\/td>.*')[0]
        else:
...
##            item['current_international_class'] = ''
        ...
        if response.xpath('//tr').re('Class\s*at\s*Publication(?:.*\n)+.*?<td valign="TOP".*?><b>(.*?)?<\/td>.*') != []:
            data = response.xpath('//tr').re('Class\s*at\s*Publication(?:.*\n)+.*?<td valign="TOP".*?><b>(.*?)?<\/td>.*')
            clean = data[0].replace('</b>','').strip()
            item['class_at_publication'] = data
        else:
            item['class_at_publication'] = ''
            
        
        #international_class
        if response.xpath('//tr').re('International\s*Class(?:.*\n)+.*?<td valign="TOP".*?>(.*?)?<\/td>.*') != []:
            item['international_class'] = response.xpath('//tr').re('International\s*Class(?:.*\n)+.*?<td valign="TOP".*?>(.*?)?<\/td>.*')[0]
        else:
        ...
        #item['related_US_patent_issue_date'] = ''
        ...
        #32 other_references - implement a test for references
        ...
        if response.xpath('//i').xpath('//text()') != []:
            ...
            ###TEST FOR REFERENCES###
            if [find_text.index('Primary Examiner:') for e in find_text if 'Primary Examiner:' in e] != []:
                beg = [find_text.index('Primary Examiner:') for e in find_text if 'Primary Examiner:' in e][0]  #debug
        ...
        if response.xpath('//html//body//text()') != []:
            text_data = response.xpath('//html//body//text()').extract()
...
#scrapy crawl uspto_patent_app_numbers -o uspto_patent_app_numbers.csv
